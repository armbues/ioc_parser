...
class OutputHandler_csv(OutputHandler):
    ...
    # Added flag which are unused but needed to make CSV output work
    def print_match(self, fpath, page, name, match, flag):
...
class OutputHandler_yara(OutputHandler):
    ...
    # Added flag which are unused but needed to make YARA output work
    def print_match(self, fpath, page, name, match, flag):
        if name in self.cnt:
            self.cnt[name] += 1
        else:
...
class OutputHandler_netflow(OutputHandler):
    ...
    # Added flag which are unused but needed to make Netflow output work
    def print_match(self, fpath, page, name, match, flag):
        data = {
            'type' : name,
            'match': match
